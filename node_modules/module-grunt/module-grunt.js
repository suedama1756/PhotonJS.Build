...
module.exports = function (grunt) {
    ...
    grunt.registerMultiTask('module', 'Build module source files', function () {
        ...
        if (!moduleFiles) {
            grunt.fail.warn('No jsm specified for module task');
        }
        ...
        moduleFiles.forEach(function (moduleFile) {
            try {
                ...
                grunt.fail.warn(_system.string.format("Error generating module '{0}', {1}.", moduleFile, e), -1);
            }
        });
    });
    ...
    grunt.registerMultiTask('closureCompiler', 'Compile using Closure Compiler.', function () {
        ...
        // process js files
        js = grunt.file.expand({filter: 'isFile'}, getClosureOption(data, 'js', 'js', true));
        if (!js.length) {
            grunt.warn('At least one javascript file must be specified.');
            return false;
        }
        command += ' --js ' + js.map(function (fileName) {
            return _system.string.format('"{0}"', fileName);
        }).join(' ');
